...
void display_processing_stats() {
  ...
  txt("SSH", curr_stats.ssh_packets);
  txt("GOPHER", curr_stats.gopher_packets);
}
...
void process_TCP(bool ipv4_type) {
  ...
  if (port(80)) {
    ...
    write_lg("SSH Protocol");
    curr_stats.ssh_packets++;
  } else if (port(70)) {
    write_lg("GOPHER Protocol");
    curr_stats.gopher_packets++;
  }
}
...
void process_packet(void *data, size_t data_size) {
  ...
  glbl_data_size = 0;
  curr_stats.num_of_packets++;
  printf("[*] Recieved packet %d, processing...\n", curr_stats.num_of_packets);
  write_lg("Packet number: %d of size %zu", curr_stats.num_of_packets,
           data_size);
  glbl_data = data;
  glbl_data_size = data_size;
  process_ethernet();
  printf("[*] Processing for packet %d done.\n", curr_stats.num_of_packets);
}
